---
title: "Internship Progress Week 11 (September 11 ~ September 16)"
date: 2026-09-16 12:00:00 +0530
categories: [Internship 2026, Progress]
tags: [internship, progress, week-11, ros2, jazzy, jderobot, gazebo, jetty, macos, apple-silicon, docker, opencv, follow-line]
published: true
---
...
## The Autonomous Control Implementation

With the simulation and perception pipeline stabilized, the F1 vehicle autonomous controller operates as follows:
1. **Perception:** Frames are captured from `/f1/camera/image_raw`, converted from BGR to HSV color space, and segmented using dual-range red thresholding to isolate the central track line.
2. **Centroid Tracking:** Spatial moments (`cv2.moments`) calculate the horizontal center of mass (`cx`) of the detected line.
3. **PID Steering:** The horizontal error between the image midpoint and the centroid feeds into a PID controller to compute the target angular velocity:
   $$\text{angular\_z} = -(K_p \cdot \text{error} + K_i \cdot \int \text{error} + K_d \cdot \frac{d\,\text{error}}{dt})$$
4. **Actuation:** Linear and angular velocities are published directly to `/f1/cmd_vel`, keeping the car aligned on the track throughout turns and straightaways.
...
Next steps include:
- Benchmarking sustained RTF across more complex multi-robot and drone environments in the Academy.
- Packaging the updated configuration and Docker recipes into an upstream pull request for JdeRobot.
